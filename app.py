...
def main():
    ...
    model_selection = f"{str(ROOT)}/{st.sidebar.selectbox('选择模型', model_files)}"
    ...
    if input_source == "本地视频":
        video = st.sidebar.file_uploader("选择输入视频", type=["mp4", "avi"], accept_multiple_files=False)
        ...
        if st.sidebar.button("开始检测"):
            ...
            detect(weights=model_selection, source=video.name, stframe=stframe, kpi1_text=kpi1_text, kpi2_text=kpi2_text, kpi3_text=kpi3_text, js1_text=js1_text, js2_text=js2_text, js3_text=js3_text, conf_thres=float(conf_thres), iou_thres=float(iou_thres), nosave=nosave, display_labels=display_labels, conf_thres_drift = float(conf_thres_drift), save_poor_frame__= save_poor_frame__, inf_ov_1_text=inf_ov_1_text, inf_ov_2_text=inf_ov_2_text, inf_ov_3_text=inf_ov_3_text, inf_ov_4_text=inf_ov_4_text, fps_warn=fps_warn, fps_drop_warn_thresh = float(fps_drop_warn_thresh))
